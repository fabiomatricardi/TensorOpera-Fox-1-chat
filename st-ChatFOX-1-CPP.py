...
# Set HF API token  and HF repo
    from langchain_community.embeddings import LlamaCppEmbeddings
    embpath = "models/e5-small-v2.Q8_0.gguf"
    embeddings = LlamaCppEmbeddings(model_path=embpath)
    print('loading e5-small-v2.Q8_0.gguf with LlamaCPP...')
    return embeddings
...
# from llama_cpp import Llama
    from langchain_community.chat_models import ChatLlamaCpp
    local_path='models/Fox-1-1.6B.Q6_K.gguf'
    fox1 = ChatLlamaCpp(
                model_path=local_path,
                n_gpu_layers=0,
                temperature=0.1,
                top_p = 0.5,
                n_ctx=8192, #up to 32k
                max_tokens=600,
                repeat_penalty=1.7,
                stop=["<|im_end|>",'<end_of_turn>',"### Instruction:","###<user>","</user>"],
                verbose=True,
                )
    print('loading Fox-1-1.6B.Q6_K.gguf with LlamaCPP...')
    return fox1
...
st.sidebar.image('fox-logo--1--3.jpg')
llm = create_chat()
